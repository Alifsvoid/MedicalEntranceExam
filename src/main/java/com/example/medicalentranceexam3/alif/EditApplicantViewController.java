...
public class EditApplicantViewController {
    @javafx.fxml.FXML
    ...
    public void handleUpdateButton(ActionEvent actionEvent) {
        ...
        String id = applicantIDTextField.getText();
        ...
        for (Applicant applicant : applicantArrayList) {
            if (applicant.getApplicantID().equals(id)) {
                // Update the applicant's data from text fields
                applicant.setName(nameTextField.getText().trim());
                applicant.setEmail(emailTextField.getText().trim());
                applicant.setPhone(phoneTextField.getText().trim());
                applicant.setDob(dobDatePicker.getValue());
        ...
        successAlert("Applicant updated successfully.");
    }
